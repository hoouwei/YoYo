...
if ($_REQUEST['act'] == '') {
    ...
    $smarty->assign('admin_id', $_SESSION['supply_id']);
...
elseif ($_REQUEST['act'] == 'top') {
    // 获得管理员设置的菜单
    $lst = array();
    $nav = $db->GetOne('SELECT nav_list FROM ' . $ecs->table('admin_user') . " WHERE user_id = '" . $_SESSION['supply_id'] . "'");
...
elseif ($_REQUEST['act'] == 'main') {
    ...
    /* 获取用户信息*/
    $user_sql = "select user_name,last_login,last_ip,email,tel,company_name from" . $ecs->table('supply_user') . "where supply_id=" . $_SESSION['supply_id'];
    $supply = $db->getRow($user_sql);
    $supply['last_login'] = date('Y-m-d H:i:s', $supply['last_login']);
